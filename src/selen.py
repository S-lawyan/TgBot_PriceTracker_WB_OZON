...
class Selen():
    ...
    async def ozon_search_tovar(self, articul, user_id):
        ...
        try:
            await asyncio.create_task(self.wait_fing_element(DRIVER, 20, (
                By.CSS_SELECTOR, 'div[data-widget="stickyContainer"]')))
        except:
            error_404 = await asyncio.create_task(self.wait_fing_element(DRIVER, 20, (
                By.CSS_SELECTOR, 'div[data-widget="webOutOfStock"]')))
            await self.clear_driver(DRIVER=DRIVER)
            return None

        # НАИМЕНОВАНИЕ ТОВАРА
        name_element = DRIVER.find_element(By.CSS_SELECTOR, 'div[data-widget="webProductHeading"]')
        product_title = name_element.text
        product['name'] = product_title

        # ИЗОБРАЖЕНИЕ
        gallery_element = await asyncio.create_task(
            self.wait_fing_element(DRIVER, 20, (By.CSS_SELECTOR, 'div[data-widget="webGallery"]')))
        image_element = gallery_element.find_element(By.TAG_NAME, 'img')
        image_url = image_element.get_attribute('src')
        product['img'] = await self.download_image(image_url)

        try:
            price_element = DRIVER.find_element(By.CSS_SELECTOR, 'div[data-widget="webPrice"]')
    ...
    async def ozon_check_price(self, articul):
        ...
        try:
            ...
            logging.error(f"OZON: Ошибка при ожидании (stickyContainer) артикула {articul}")
            try:
                ...
                logging.error(f"OZON: Ошибка при ожидании (div[@data-widget='error']) артикула {articul}")
                DRIVER.delete_all_cookies()
                return None
    ...
    async def wb_search_tovar(self, articul, user_id):
        ...
        try:
            await asyncio.create_task(self.wait_fing_element(DRIVER, 20, (
                By.XPATH, '//section[@class="product-page__details-section details-section"]')))
        except:
            error_404 = WebDriverWait(DRIVER, 20).until(
                EC.presence_of_element_located((By.XPATH, "//h1[@class='content404__title']")))
            await self.clear_driver(DRIVER=DRIVER)
            return None

        # НАИМЕНОВАНИЕ ТОВАРА
        name_element = DRIVER.find_element(By.CSS_SELECTOR, 'div[class="product-page__header"]')
        product_title = name_element.text
        product['name'] = product_title.replace('\n', ' ')

        # ИЗОБРАЖЕНИЕ
        image_element = await asyncio.create_task(
            self.wait_fing_element(DRIVER, 20, (By.CSS_SELECTOR, 'div[class="zoom-image-container')))
        image = image_element.find_element(By.TAG_NAME, 'img')
        image_url = image.get_attribute('src')
        product['img'] = await self.download_image(image_url)

        try:
            price_element = DRIVER.find_element(By.XPATH, ".//ins[@class='price-block__final-price']")
            price = price_element.get_attribute("textContent")
            price = ''.join(char for char in price if char.isdigit())
        except Exception as exc:
            sold_out_element = DRIVER.find_element(By.XPATH, ".//p[@class='sold-out-product']")
            price = False
    ...
    async def wb_check_price(self, articul):
        ...
        try:
            ...
            logging.error(f"WB: Ошибка при ожидании (product-page__details-section details-section) артикула {articul}")
            try:
                error_404 = WebDriverWait(DRIVER, 20).until(
                    EC.presence_of_element_located((By.XPATH, "//h1[@class='content404__title']")))
                DRIVER.delete_all_cookies()
                ...
                logging.error(f"WB: Ошибка при ожидании (content404__title) артикула {articul}")
                DRIVER.delete_all_cookies()
                return None
    ...
    async def wait_fing_element(self, driver, timeout, locator):
        ...
        element = await asyncio.get_event_loop().run_in_executor(None, lambda: WebDriverWait(driver, timeout).until(
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i in range(retry_count):
                try:
                    ...
                    logging.error(f"Ошибка подключения: {e}. Повторная попытка ({i + 1}/{retry_count})...")
                    await asyncio.sleep(1)  # Пауза перед повторной попыткой
                    continue
            else:
                logging.error(f"Ошибка подключения: {e} --- {url} --- Превышено количество попыток.")
                await self.close_session(session=session)
                return b''
