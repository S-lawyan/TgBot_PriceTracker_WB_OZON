...
class DataBase():
    ...
    def create_scheme(self):
        ...
        with self.connect as connect:
            try:
                connect.execute(f"""CREATE TABLE users (user_id	INTEGER NOT NULL UNIQUE)""")
                connect.commit()
            except Exception as e:
                print(f"При создании таблицы БД возникло исключение: {e}")

            try:
                connect.execute(f"""CREATE TABLE positions (articul	INTEGER NOT NULL,
                                                            name TEXT,
                                                            price INTEGER,
                                                            img BLOB,
                                                            user_id INTEGER,
    ...
    async def check_user(self, user_id):
        with self.connect as connect:
            result = bool(connect.execute(f""" SELECT user_id FROM users WHERE user_id = {user_id} """).fetchone())
        return result
