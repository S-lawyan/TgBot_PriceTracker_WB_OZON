...
async def command_start(message: types.Message, state: FSMContext):
    ...
    if flag is False:
        sent_message = await message.answer(text="Мой хозяин запрещает мне разговаривать с незнакомцами 😒\n",
                                            reply_markup=kb_password)
        async with state.proxy() as data:
            data['sent_message'] = sent_message
    else:
        await message.answer(text="Отправь мне <b>артикул</b> товара или <b>ссылку</b> на него",
...
async def processing_arcicul(message: types.Message, state: FSMContext):
    ...
    if re.match(r'^\d{5,}$', msg):
        articul = msg
        sent_message = await message.answer(text="Выберите источник", reply_markup=await enter_source_kb(articul))
        async with state.proxy() as data:
            data['sent_message'] = sent_message
    ...
    # ОБРАБОТКА ССЫЛОК -> автоматическое определение источника
        ...
        if match:
            ...
            await message.reply("Ссылка не определяется, попробуйте прислать <b>артикул</b>.")
            return
    ...
    if articul is not None:
        ...
        if source == 'wb':
            result = await selen.wb_search_tovar(articul=articul, user_id=message.from_user.id)
        elif source == 'ozon':
            result = await selen.ozon_search_tovar(articul=articul, user_id=message.from_user.id)
        else:
            await message.answer("Произошло что-то не понятное с определением источника ссылки")
            return
        ...
        if result is None:
            ...
            if result['img'] == b'':
                ...
                price = "Нет в наличии" if result['price'] == False else str(result['price']) + ' ₽'
                sent_message = await message.answer_photo(photo=result['img'],
...
async def enter_source(call: types.CallbackQuery, state: FSMContext):
    ...
    if source == 'wb':
        result = await selen.wb_search_tovar(articul=articul, user_id=call.message.from_user.id)
        source_ = 'Wildberries 🟣'

    elif source == 'ozon':
        result = await selen.ozon_search_tovar(articul=articul, user_id=call.message.from_user.id)
        source_ = 'OZON 🔵'

    else:
        await call.message.answer("Произошло что-то не понятное с определением источника артикула")
        await state.finish()
        return
...
async def add_position(call: types.CallbackQuery, state: FSMContext):
    async with state.proxy() as data:
        ...
        sent_message = data['sent_message']
        source = data['source']
    flag = await db.check_position(articul=int(result['articul']), user_id=call.from_user.id, source=source)
    if flag is True:
        await bot.send_message(call.from_user.id, f"⚠️Товар с таким артикулом уже есть среди отслеживаемых")
        ...
        await db.save_position(articul=int(result['articul']), name=result['name'], price=price, date_time=time_now,
                               user_id=call.from_user.id, img=result['img'], source=source)
        await state.finish()
        await sent_message.edit_reply_markup(reply_markup=None)
...
async def my_positions(message: types.Message, state: FSMContext):
    try:
        ...
        wb_position_list = await db.get_all_position(user_id=message.from_user.id, source='wb')
        ozon_position_list = await db.get_all_position(user_id=message.from_user.id, source='ozon')
        ...
        if len(wb_position_list) == 0:
            ...
            for item in wb_position_list:
                price = str(item[2]) + " ₽" if str(item[2]) != "Нет в наличии" else str(item[2])
        ...
        logging.error(f"Исключение при получении списка всех позиций: {e} -- user -- {message.from_user.id}")
...
async def processing_delete(message: types.Message, state: FSMContext):
    ...
    if not re.match(pattern, msg):
        await sent_message.edit_reply_markup(reply_markup=None)
        sent_message = await message.answer("⚠️В артикуле(ах) допущена ошибка, повторите ввод", reply_markup=kb_cancel)
        async with state.proxy() as data:
            data['sent_message'] = sent_message
        return
        ...
        for articul in articul_list:
            ...
            if flag == False:
                ...
                try:
                    await db.dell_position(articul=articul, user_id=message.from_user.id, source=source)
                    ...
                    continue
        if deleted_list != "":
            await message.answer(f"✅ Следующие позиции были успешно удалены:\n<b>{deleted_list}</b>")
        await state.finish()


def register_handlers_client(dp: Dispatcher):
    dp.register_message_handler(command_start, commands=['start'], state=None)
    dp.register_message_handler(processing_arcicul, content_types=types.ContentType.TEXT, state=None)
    dp.register_message_handler(processing_delete, content_types=types.ContentType.TEXT,
